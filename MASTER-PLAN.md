...
| Jul 2026 | **Spanish Listening Mode shipped (Marcelle).** New 3rd launch profile 🇪🇸 "Español (Marcelle)" — keeps her Spanish alive over summer break (out of dual-language school). All-Spanish level rotation, **listening-first**: 57-word K/1st-grade vocab bank (`SPANISH_VOCAB`, themed: animales/comida/naturaleza/colores/cuerpo). Two round types reusing the tap renderer + **native Spanish TTS** (Raul/Sabina): 'listen' (hear ES word → tap picture, 5/level) + 'picture' (see picture → tap ES word, 3/level). 8 questions/level; English translation + word re-spoken on correct; wrong answer re-speaks ES. **Marcelle's arcade mini-games play in between** (fall through — not Nora's memory match). Separate `spanish_*` save slot. Mirrors the Nora Quest profile pattern. `node --check` passes; SW cache→v7. **Awaiting parent/kid test.** Cosmetic TODO: title still says "Marcelle's…". |
